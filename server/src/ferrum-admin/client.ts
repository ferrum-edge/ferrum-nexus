 ...
 *
 * Consumer and proxy mutations **must** go through this: `PUT /consumers/{id}`
 * and `PUT /proxies/{id}` are whole-resource replaces with no concurrency token
 * (`ref-edge-admin.md` §7.2), so two concurrent GET→edit→PUT round trips
 * silently lose one edit — an ACL group revocation restored, or a proxy left
 * with no authentication plugin.
 *
 ...
 *
 * A caller that cannot get the lease within `waitMs` gets a `CONFLICT` naming
 * the other instance, not a silent overwrite.
 */
