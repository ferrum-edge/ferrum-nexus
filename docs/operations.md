...
| `NEXUS_RATE_LIMIT_ENABLED`      | `true`                                       | Installs the 20 req/min limiter on `/api/auth/*` and the 120 req/min limiter on `/api/health*` (both per client IP), and the 30 req/min limiter on the mutating `/api/apis/*` routes (per account). Forced off when `NEXUS_ENV=test`. See [Abuse controls](#abuse-controls).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
