...
| `EDGE_ERROR`         | 502  | Ferrum Edge Admin API returned an error response. `details` contains the upstream HTTP `status`; the operator-facing upstream error text is logged but never returned to the caller. |
