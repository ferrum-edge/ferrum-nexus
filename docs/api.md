...
| `CREDENTIAL_INVALIDATION_REQUIRED` | 409  | Refused: the change would strand live gateway credentials and the request did not say it accepts that. Today only a `PATCH /api/apis/:id` that moves `auth_plugin`. `details` is `{ field, current_auth_plugin, requested_auth_plugin, credential_type, active_credentials, confirm_field }`; resending with `confirm_credential_invalidation: true` makes the change and revokes the credentials it names.                   |
...
changes. `bootstrap_required` is never memoised: it is read live, so a founder
seated on any instance is reflected everywhere at once. Concurrent anonymous
requests coalesce onto one in-flight count query per instance, which bounds the
database work a burst can cause without retaining the answer. The TTL bounds
...
| `auth_plugin`                     | attaches and associates the new auth plugin config before detaching and deleting the old one. **Refused with `409 CREDENTIAL_INVALIDATION_REQUIRED` while live credentials of the outgoing flavour exist**; send `confirm_credential_invalidation: true` to make the change anyway, which revokes those credentials on the gateway and in the portal. Every grantee is notified either way                                                                                 |
...
| `confirm_credential_invalidation` | acknowledgement, not a setting: it only means anything alongside an `auth_plugin` change, and only as `true`. See that row                                                                                                                                                                                                                                                                                                                                                 |
...
`409 CREDENTIAL_INVALIDATION_REQUIRED` (an `auth_plugin` change that would
strand live credentials, without `confirm_credential_invalidation: true`),
`502 EDGE_ERROR`.

> **Changing `auth_plugin` breaks every credential issued for the old one.**
> Edge runs one authentication plugin per proxy, so the moment the swap lands
> every key of the outgoing flavour is a `401` on this API — a break the client
> sees before the portal does. Nexus therefore refuses the change while any such
> credential is live, and says how many there are; the provider re-sends with
> `"confirm_credential_invalidation": true` to go ahead. That path revokes those
> credentials on the gateway and in the portal, writes a `credential.revoke` row
> for each and one `api.credentials_invalidated` summary, and notifies every
> grantee to issue a replacement. The count covers the accounts holding an
> active grant on this API plus its `nexus-test-<api_id>` consumer: a credential
> hangs off a consumer, not an API, so an API published with `requestable: false`
> — which gates nobody — has no enumerable callers, and its holders get the
> notification rather than a refusal. A grantee's revoked credential was serving
> their other APIs of that flavour too, and stops serving those as well.
