...
| A DB column or table | Edit the three `server/src/db/migrations/001_initial*.sql` baselines in place during buildout and update their checksums in `server/src/db/released-migrations.ts`; after the first supported release, add a forward migration instead ([operations](operations.md#schema-versioning-and-upgrades)). Update MongoDB indexes, update `NexusStore`, implement in **all four** adapters, run the cross-adapter smoke tests. |
