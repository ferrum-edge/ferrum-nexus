...
| A DB column or table | `001_initial` is frozen (`v0.1.0`): add a forward migration with the next id for all three SQL dialects and a `MONGO_MIGRATIONS` step, never an edit to a released one ([operations](operations.md#schema-versioning-and-upgrades)). Update `NexusStore`, implement in **all four** adapters, run the cross-adapter smoke tests. |
