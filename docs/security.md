...
| Spec injection | OpenAPI documents are parsed locally, external `$ref` URLs are rejected, Swagger 2.0 specs are normalized to OpenAPI 3.x, and governance policy can block or warn on standards violations before Edge mutation. |
...
- `api.publish`, `api.publish_with_exception`, `api.spec_replace`,
  `api.settings_update`, `api.delete`, `api.import`, `admin.god_delete_api`
...
- `admin.user_approve_registration`, `admin.user_deny_registration`
- `policy.update`, `policy_exception.create`, `policy_exception.approve`,
  `policy_exception.deny`
