...
| `credential.revoke_rollback` | `credential` | A retirement whose gateway delete provably never applied — the array still held every entry — was withdrawn: the row went back to `active` instead of on to `revoked`. The completion of that call's `credential.revoke_start`, committed with the move back; if that transaction cannot commit, the move is retried on its own and the row follows best-effort, unless the transaction is found to have committed with only its acknowledgement lost. `details`: `credential_type`, `consumer_id`, `last4`, `operation` (`revoke` \| `rotate`), `cause`, plus `owner_user_id` when an admin acted on somebody else's credential.                                                                                                                                                                                                                                                                                                          |
