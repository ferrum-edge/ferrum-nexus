...
Upstream error text from Edge is always logged and never echoed to the browser.
Those operator-facing messages can name internal hosts and settings regardless
of their HTTP status; callers see only `EDGE_ERROR` / `EDGE_UNAVAILABLE` with a
generic message. This boundary is enforced in `classify()` in
`ferrum-admin/client.ts`; nothing else in the codebase reads an Edge error body.
