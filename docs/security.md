...
deletion, a palette plugin removal, a credential revocation — first commits an
intent row (`application.delete_start`, `api.delete_start`,
`api.plugin_remove_start`, `credential.revoke_start`) and writes its completion
row with the local change;
if that last commit fails, the portal rows remain for the operation to be
repeated and the intent row names who started it. A repeat that finds the
gateway side already gone copies what an earlier attempt's intent row recorded
into its completion row and marks it `resumed: true`, rather than reporting
...
`api.spec_update`, `api.spec_rollback` and `api.gateway_restore` are still
recorded after their deployment commits.
...
| `credential.revoke_start`    | `credential` | A revocation is about to delete a credential entry from Edge. Committed with the row's move to `retiring`, before the gateway delete, so a failure to record it stops the revocation before anything changed. `details` as on `credential.revoke`. A start with no completion is a revocation whose record failed after the delete: the row stays `retiring`, and repeating the revocation settles it and writes `credential.revoke`. A rotation at the per-type cap, which deletes the old entry before appending its replacement, commits one too, with `operation: "rotate"` (plus `owner_user_id` when an admin rotated somebody else's credential); its completion is the `credential.rotate` row, and a start with none is a rotation whose replacement could not be created after the old key was gone. |
