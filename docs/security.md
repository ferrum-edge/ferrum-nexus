...
`disable-user` with `revoke_grants` reports the sweep grant by grant. When any
...
| `god.disable_user`       | `user`      | An account was disabled, its sessions destroyed and its gateway identity stripped. `details`: `reason`, `revoked_grants`, `terminated_sessions`, `previous_status`, and the same `gateway_*` keys as `user.disable`. When a step after the committed disable fails, both this row and `user.disable` carry `failed_steps` (`revoke_grants` / `terminate_sessions`) and the request answers with that error; repeating it re-runs every step. |
