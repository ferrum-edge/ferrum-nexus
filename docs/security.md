...
Gateway work that cannot be rolled back — a deletion, a palette plugin removal,
a credential revocation — first commits an intent row
(`application.delete_start`, `api.delete_start`, `api.plugin_remove_start`,
`credential.revoke_start`) and writes its completion row with the local change;
...
| `access.revoke_rollback`  | `grant`          | A revocation claimed the grant but the gateway would not drop the ACL group; records what was undone, in the transaction that puts the grant back when it does. `details`: api id, user id, `acl_group`, `cause`, `grant_restored`. `grant_restored: false` means the portal says revoked while the group may still be live — investigate.                                                                                                                                                                                                                                     |
