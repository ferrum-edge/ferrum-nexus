...
| `credential.revoke_rollback` | `credential` | A retirement whose gateway delete provably never applied — the array still held every entry — was withdrawn: the row went back to `active` instead of on to `revoked`. The completion of that call's `credential.revoke_start`, committed with the move back; if the lease fence refuses that transaction the row stays `retiring`, since another instance may already have settled it; any other failure retries the move on its own, only while the row is still `retiring`, and the row follows best-effort unless the transaction committed with only its acknowledgement lost. `details`: `credential_type`, `consumer_id`, `last4`, `operation` (`revoke` \| `rotate`), `cause`, plus `owner_user_id` when an admin acted on somebody else's credential.                                                                                                                                                                             |
