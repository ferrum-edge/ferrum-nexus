...
**Privileged transitions and deletions commit with their audit rows.** A role
change, enable, disable or administrative account edit (ordinary or god-mode), a
gateway-revocation retry, and an application or API deletion write their rows
through the store transaction that makes the change, so a failed audit insert
rolls the change back instead of leaving it applied and unaudited behind a
`500` — and the repeated request then finds a real change to make and records
it. A deletion's gateway work cannot be rolled back, so it first commits an
intent row (`application.delete_start`, `api.delete_start`) and writes its
completion row with the local delete; if that last commit fails, the portal rows
remain for the delete to be retried and the intent row names who started it.
Work that runs after a committed transition — a disable's immediate gateway
revocation, a god-mode grant sweep — is recorded as an outcome row
(`user.gateway_teardown_complete`, `god.disable_user_complete`) once it has run.
The actions this applies to are `TRANSACTIONAL_AUDIT_ACTIONS` in
[`server/src/audit/service.ts`](../server/src/audit/service.ts), and
`server/src/test/transactional-audit.test.ts` fails when one of them is recorded
...
| `credential.revoke`          | `credential` | A credential was deleted from Edge and marked revoked. `details`: type, consumer id, `last4` — plus, when the revocation was a consequence rather than a request, `reason`, `api_id` and the `auth_plugin` pair behind it, which is what tells an API retiring a key of its own apart from an owner revoking theirs. Only the API's `nexus-test-<api_id>` credentials are ever revoked that way; a grantee's key is left alone. |
