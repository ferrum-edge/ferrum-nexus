...
`001_initial` is the baseline `v0.1.0` froze. `db/released-migrations.ts`
records each released migration's per-backend checksum; a schema change is a new
forward migration with a higher id in every backend, and CI rejects an edit to a
released one ([operations](operations.md#schema-versioning-and-upgrades)). The server build copies SQL assets to
...
4xx parse/validation category, excluding authentication failures), and
`EDGE_PROTOCOL_ERROR` (an invalid HTTP/JSON response, including malformed UTF-8).
Protocol diagnostics contain
